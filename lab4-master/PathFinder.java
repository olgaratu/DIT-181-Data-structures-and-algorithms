 ...
 * 
 * There are several methods for finding paths, and they all return a
 * PathFinder.Result object.
 */
public class PathFinder<Node> {
    ...
    /**
     * Perform a random walk in the graph, hoping to reach the goal. Warning: this
     * method will give up of the random walk reaches a dead end or after one
     * million iterations. So a negative result does not mean there is no path.
     * 
     * @param start the start node
     * @param goal  the goal node
     */
    ...
    public Result searchUCS(Node start, Node goal) {
        ...
        /******************************
         * TODO: Task 1a+c * Change below this comment *
         ******************************/
        pqueue.add(new PQEntry(start, 0, 0, null));
        ...
        while (!pqueue.isEmpty()) {
            ...
            if (!visitedNodes.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : neighbours) {
                    // if (!visitedNodes.contains(edge.to())) {
                    double costToNext = entry.costToHere + edge.weight();
                    pqueue.add(new PQEntry(edge.to(), costToNext, 0, entry));
                    // }
    ...
    public Result searchAstar(Node start, Node goal) {
        int iterations = 0;
        /******************************
         * TODO: Task 3 * Change below this comment *
         ******************************/
        Queue<PQEntry> pqueue = new PriorityQueue<>(
                Comparator.comparingDouble(entry -> entry.costToHere + entry.costToGoal));
    ...
    /**
     * Extract the final path from start to goal, from the final priority queue
     * entry.
     * 
    ...
    private List<Node> extractPath(PQEntry entry) {
        LinkedList<Node> path = new LinkedList<>();
        /******************************
         * TODO: Task 1b * Change below this comment *
         ******************************/
        PQEntry curruntEntry = entry;
        while (curruntEntry != null) {
            path.addFirst(curruntEntry.node);
            curruntEntry = curruntEntry.backPointer;
        }
        return path;
    }

    /**
     * Entries to put in the priority queues in {@code searchUCS} and
     * {@code searchAstar}.
     */
    private class PQEntry {
        ...
        /***********************************
         * TODO: Task 3 * Change below this comment, * for example, to add new fields. *
         **********************************/
        public final double costToGoal;

        PQEntry(Node n, double c, double ctg, PQEntry bp) {
            node = n;
            costToGoal = ctg;
            costToHere = c;
            backPointer = bp;
        ...
        public String toString() {
            ...
            if (success) {
                ...
                if (len == 0)
                    ...
                    Function<List<Node>, String> joinPath = path -> path.stream().map(Node::toString)
                            .collect(Collectors.joining(" -> "));
                    if (len < 10)
                        w.println("Path: " + joinPath.apply(path));
                    else
                        w.println("Path: " + joinPath.apply(path.subList(0, 5)) + " -> ... -> "
