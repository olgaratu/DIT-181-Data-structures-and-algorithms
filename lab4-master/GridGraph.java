 ...
 *
 * Some characters are passable, and some denote obstacles. A node (also called
 * "state") is a coordinate, i.e., a pair of integers; which is defined as the
 * internal class GridGraph.Coord. You can move in the 4 normal directions, as
 * well as diagonally. The edge costs are 1.0 (for N/S/E/W) and Math.sqrt(2)
 * (for diagonal movement). The graph can be read from a simple ASCII art text
 * file.
 */
...
public class GridGraph implements DirectedGraph<GridGraph.Coord> {
    ...
    // | - + walls
    // space passable
    // Note: "-" must come last in allowedChars, because we use it unescaped in a
    // regular expression.
    ...
    /**
     * Creates a new graph with edges from a text file. The file describes the graph
     * as ASCII art, in the format of the graph files from the Moving AI Lab.
     * 
     * @param file path to a text file
     */
    public GridGraph(String file) throws IOException {
        grid = Files.lines(Paths.get(file)).filter(line -> line.matches("^[" + allowedChars + "]+$"))
     ...
     * @return the guessed best cost for getting from {@code p} to {@code q} (the
    ...
    public double guessCost(Coord p, Coord q) {
         ...
         ******************************/
        double estimateCost = Math.sqrt((p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
    ...
    /**
     * @return a string representation of this graph, including some random
    ...
    public String toString() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            w.println(p + " --> " + edges.stream().map(e -> String.format("%s[%.1f]", e.to(), e.weight()))
