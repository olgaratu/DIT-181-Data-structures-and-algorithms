...
public class AutocompleteGUI extends JFrame {
    ...
    private class AutocompletePanel extends JPanel {
        ...
        public AutocompletePanel(String filename) {
            ...
            Term[] terms = null;
            try {
                terms = Files.lines(Paths.get(filename)).map(line -> line.trim().split("\\s+"))
                        .map(pair -> new Term(pair[1], Long.valueOf(pair[0]))).toArray(size -> new Term[size]);
            } catch (Exception e) {
                    ...
                    for (int i = 0; i < results.length; i++) {
                        ...
                        if (splitted.length < 2) {
                            throw new RuntimeException("allMatches() returned"
                                    + " an array with an ill-formatted entry:" + " '" + next + "'");
                        }
     ...
     *             the integer that defines the maximum number of objects in the
     *             dropdown menu
     */
