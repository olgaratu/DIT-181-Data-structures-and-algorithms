...
public class Quick {
    ...
    private void sort(int[] a, int lo, int hi) {
        ...
        if (hi-lo<insertionSortCutoff) {
            ...
            //if the array become to small its better to switch to insertion sort
            /*Insertion sort*/
    ...
    private int partition(int[] a, int lo, int hi) {
        ...
        if (useMedianOfThree) {
           ...
           //int randomNum = ThreadLocalRandom.current().nextInt(lo, hi + 1);
           // medianOfThree(a,lo,hi,(lo+hi)/2);
...
//        int low1 = Math.min(a[i], a[j]);
//        int low2 = Math.min(a[j], a[k]);
//        int low = Math.min(low1, low2);
//        int high1 = Math.max(a[i], a[j]);
//        int high2 = Math.max(a[j], a[k]);
